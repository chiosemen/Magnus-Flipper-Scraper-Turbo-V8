...
export const createScrapeTask = async (payload: JobPayload) => {
  ...
  const isDemo = !!payload.meta?.demo;
  const queueName = isDemo ? process.env.DEMO_GCP_QUEUE_NAME : queue;
  if (isDemo && !queueName) {
    throw new Error('DEMO_GCP_QUEUE_NAME is required for demo mode dispatch');
  }

  const parent = client.queuePath(project || 'unknown', location, queueName || 'unknown');
