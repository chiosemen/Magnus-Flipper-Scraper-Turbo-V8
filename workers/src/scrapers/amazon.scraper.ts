  ...
  async parseSearchResults(page: Page): Promise<{ listings: CreateDeal[]; nextPageUrl: string | null }> {
    ...
    for (const el of elements) {
      try {
        ...
        const availabilityEl = await el.$('.a-color-success, .a-color-state');
        const availabilityText = (await availabilityEl?.innerText() || '').toLowerCase();
        if (availabilityText.includes('out of stock')) continue;
