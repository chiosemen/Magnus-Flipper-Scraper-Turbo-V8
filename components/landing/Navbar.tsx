  ...
  return (
    ...
    >
      ...
      {isOpen && (
        ...
        >
          <a href="#features" className="block text-slate-400" onClick={() => setIsOpen(false)}>Features</a>
          <a href="#pricing" className="block text-slate-400" onClick={() => setIsOpen(false)}>Pricing</a>
          <Link to="/login" className="block text-white font-bold" onClick={() => setIsOpen(false)}>Log In</Link>
        </motion.div>
      )}
    </motion.nav>
  );
