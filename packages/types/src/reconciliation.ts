...
export type ReconciliationSeverity = z.infer<typeof ReconciliationSeverityEnum>;
...
export type CustomerReconciliation = z.infer<typeof CustomerReconciliationSchema>;
...
export type CustomerInvoiceQuery = z.infer<typeof CustomerInvoiceQuerySchema>;
...
export interface BilledData {
  ...
  totalAmount: number;
  componentTotals: Map<string, { units: number; amount: number }>;
}

export interface MeasuredData {
  userId: string;
  usage: UsageTotals;
  componentTotals: Map<string, { units: number; cost: number }>;
}
...
export const SEVERITY_ORDER: Record<ReconciliationSeverity, number> = {
  OK: 0,
  WARN: 1,
  CRITICAL: 2,
};
...
export function getHigherSeverity(
  a: ReconciliationSeverity,
  b: ReconciliationSeverity
): ReconciliationSeverity {
  return SEVERITY_ORDER[a] > SEVERITY_ORDER[b] ? a : b;
}

export function calculateDeltaPct(billed: number, measured: number): number {
  if (measured === 0) return billed === 0 ? 0 : 100;
  return ((billed - measured) / measured) * 100;
}

export function classifySeverity(deltaAmount: number, deltaPct: number): ReconciliationSeverity {
